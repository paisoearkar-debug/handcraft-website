...
$("list").innerHTML=(data||[]).map(p=>`<div class="project-row"><img src="${p.image_url||"../assets/placeholder.jpg"}"><div><b>${esc(p.title)}</b><div>${esc(p.category||"")} · ${p.published?"Published":"Hidden"}</div></div><div><button onclick='editProject(${JSON.stringify(p)})'>Edit</button> <button class="danger" onclick="deleteProject('${p.id}')">Delete</button></div></div>`).join("")||"No projects yet."}
